  ...
  for(int z=0; z<((int)dimensions[2]); z++){
      for(int y=0; y<((int)dimensions[1]); y++){
          for(int x=0; x<((int)dimensions[0]); x++){
              New[index]=0; // Initialisation to 0
              if(Old[index]>0){
                  New[index]=CCcounter; // if value somewhere, put CCcounter instead that counts the numbers of used voxels
                  CCcounter++;
                }
              index++;
            }
...
// Allocate CClist with number of non zero voxels in Old
  ...
  while(numbchanges!=0){
      ...
      numbchanges=0;
      int currindex=0;
      for(int z=1; z<((int)dimensions[2]-1); z++){ /* Taking care of possible definition problems at the boundary*/
          for(int y=1; y<((int)dimensions[1]-1); y++){
              for(int x=1; x<((int)dimensions[0]-1); x++){
                  index = z*dimensions[1]*dimensions[0]+y*dimensions[0]+x;
                  if(Old[index]>0 && CClist[New[index]]>0){ // if Old[index]>0, then New[index]>0
                      tempmin=CClist[New[index]];
                        /*Look at neighbours in all 3 directions to determine smallest count of non zero value in Old*/
...
void Close_Forground_ConnectComp(void * Old_void, void * New_void, ImageSize * Currentsize){
  ...
  //  **********    Foreground    ***********
  index=0;
  // Everything that was non zero before is set to zero, the rest to 1
  for(int z=0; z<((int)dimensions[2]); z++){
      for(int y=0; y<((int)dimensions[1]); y++){
          for(int x=0; x<((int)dimensions[0]); x++){
              tempimg[index]=0;
  ...
  index=0;
  // After that CCCounter have in memory the number of elements in the background
  for(int z=0; z<((int)dimensions[2]); z++){
      for(int y=0; y<((int)dimensions[1]); y++){
          for(int x=0; x<((int)dimensions[0]); x++){
              tempimg[index]=0;
...
void Largest_ConnectComp(void * Old_void, void * New_void, ImageSize * Currentsize){
  ...
  index=0;
  // Counts the non background voxels
  for(int z=0; z<((int)dimensions[2]); z++){
      for(int y=0; y<((int)dimensions[1]); y++){
          for(int x=0; x<((int)dimensions[0]); x++){
              tempimg[index]=0;
...
//Enables the dilation of the image given a certain value for the kernel of dilation
void Dillate(float * Image,
  ...
  if(Buffer == NULL){
      fprintf(stderr,"* Error when allocating Buffer in void Dillate(): Not enough memory\n");
      exit(1);
    }
      ...
      for(xyzpos[2]=0;xyzpos[2]<dimensions[2];xyzpos[2]++){
          for(xyzpos[1]=0;xyzpos[1]<dimensions[1];xyzpos[1]++){
              for(xyzpos[0]=0;xyzpos[0]<dimensions[0];xyzpos[0]++){
                  ...
                  Buffer[index]=tmpvalue; // In Buffer at index, we put maximum value of the kernel considered
                  index++;
                }
            }
        }
      for(int i=0; i<(dimensions[1]*dimensions[0]*dimensions[2]); i++){
          Image[i]=Buffer[i];// Buffer copied back on image
        }
    }
...
// Similar to Dilation but instead of taking the maximum of the kernel, the minimum is put instead
void Erosion(float * Image,
  ...
  float * Buffer = new float [(dimensions[1])*(dimensions[0])*(dimensions[2])];
  if(Buffer == NULL){
      fprintf(stderr,"* Error when alocating Buffer in void Erosion(): Not enough memory\n");
      exit(1);
    }
  ...
  for(int currentdirection=0;currentdirection<3;currentdirection++){ //Buffer aint each direction
      ...
      for(xyzpos[2]=0;xyzpos[2]<dimensions[2];xyzpos[2]++){
          for(xyzpos[1]=0;xyzpos[1]<dimensions[1];xyzpos[1]++){
              for(xyzpos[0]=0;xyzpos[0]<dimensions[0];xyzpos[0]++){
                  tmpvalue=1e32;
                  for(int shift=((xyzpos[currentdirection]<kernel)?-xyzpos[currentdirection]:-kernel); shift<=((xyzpos[currentdirection]>=(dimensions[currentdirection]-kernel))?(int)dimensions[currentdirection]-xyzpos[currentdirection]-1:kernel);shift++){
                      if(Image[index+shift*shiftdirection[currentdirection]]<tmpvalue){ // looks for minimum value in kernel considered
                          tmpvalue=Image[index+shift*shiftdirection[currentdirection]];
                        }
      ...
      for(int i=0; i<(dimensions[1]*dimensions[0]*dimensions[2]); i++){
          Image[i]=Buffer[i]; // Copy back into image
        }
    }
  delete [] Buffer;
